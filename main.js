...
window.onload = () => {
	...
	const wireframeFragmentShader = compileShader(
	...
	if (!gl.getProgramParameter(wireframeProgram, gl.LINK_STATUS)) {
		console.error(
			"Wireframe program linking error:",
			gl.getProgramInfoLog(wireframeProgram)
		);
	}
	...
	const wireframeUniformLocations = {
		...
		projectionParams: gl.getUniformLocation(
			wireframeProgram,
			"uProjectionParams"
		),
	};
	...
	gl.bufferData(
		gl.ARRAY_BUFFER,
		new Float32Array(bottle.positions),
		gl.STATIC_DRAW
	);
	...
	gl.bufferData(
		gl.ARRAY_BUFFER,
		new Float32Array(bottle.normals),
		gl.STATIC_DRAW
	);
	...
	gl.bufferData(
		gl.ARRAY_BUFFER,
		new Float32Array(bottle.textureCoords),
		gl.STATIC_DRAW
	);
	...
	gl.bufferData(
		...
		new Uint16Array(bottle.indices),
		gl.STATIC_DRAW
	);
	...
	function render(now) {
		...
		gl.uniform4fv(uniformLocations.cameraParams, [
			cameraAngleX,
			cameraAngleY,
			cameraDistance,
			0.0,
		...
		const near = 0.1;
		const far = 100.0;
		gl.uniform4fv(uniformLocations.projectionParams, [
			fov,
			aspectRatio,
			near,
			far,
		...
		gl.uniform4fv(wireframeUniformLocations.cameraParams, [
			cameraAngleX,
			cameraAngleY,
			cameraDistance,
			0.0,
		]);
		gl.uniform4fv(wireframeUniformLocations.projectionParams, [
			fov,
			aspectRatio,
			near,
			far,
		...
		gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
		gl.vertexAttribPointer(
			wireframeAttribLocations.vertexPosition,
			3,
			gl.FLOAT,
			false,
			0,
		...
		gl.bindBuffer(gl.ARRAY_BUFFER, normalBuffer);
		gl.vertexAttribPointer(
			wireframeAttribLocations.vertexNormal,
			3,
			gl.FLOAT,
			false,
			0,
		...
		if (numIndices % 3 === 0) {
			...
			console.warn(
				"Wireframe rendering: bottle.indices.length is not a multiple of 3."
			);
		}
	...
	canvas.addEventListener("mousemove", (event) => {
		...
		cameraAngleX = Math.max(
			-Math.PI / 2 + 0.01,
			Math.min(Math.PI / 2 - 0.01, cameraAngleX)
		);
